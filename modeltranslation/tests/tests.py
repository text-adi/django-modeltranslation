...
class SpecificLanguageCodeModelTestCase(ModeltranslationTestBase):
    new_language = (('de', 'Deutsch'), ('en', 'English'), ('ua', 'Ukrainian'))

    def test_using_custom_language_code(self):
        with override_settings(LANGUAGES=self.new_language):
            self.assertEqual(self.new_language, django_settings.LANGUAGES)

            models.SpecificLanguageCodeModel.objects.create(title_en="foo", title_de="bar", title_ua="bag")
            foo = models.SpecificLanguageCodeModel.objects.filter(title_ua="bag")

            assert 1 == foo.count()

            call_command('update_translation_fields', 'tests', verbosity=0)

    def test_using_basic_language_code(self):
        models.SpecificLanguageCodeModel.objects.create(title_en="foo", title_de="bar", title_ua="bag")
        foo = models.SpecificLanguageCodeModel.objects.filter(title_ua="bag")
        assert 1 == foo.count()
